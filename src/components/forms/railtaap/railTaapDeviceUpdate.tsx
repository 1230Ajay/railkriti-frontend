...
const RailTaapDeviceUpdate: React.FC<DeviceUpdateFormProps> = ({ device, onClose }) => {
  ...
  const [meanTemp, setMeanTemp] = useState('');
  const [deProcessingTemp, setDeProcessingTemp] = useState('');
  const [installedAt, setInstalledAt] = useState('MAIN_LINE');
  ...
  const fetchDeviceData = async () => {
    ...
    try {
      ...
      if (device) {
        ...
        setMeanTemp(device.mean_temp.toString());
        setDeProcessingTemp(device.de_stress_temp.toString());
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    const formData = {
      ...
      location: location,
      mean_temp: Number(meanTemp),
      de_stress_temp: Number(deProcessingTemp),
...
export default RailTaapDeviceUpdate;
