...
const AlertPage: React.FC = ():JSX.Element => {
  ...
  const fetchAlerts = async () => {
    try {
      const response = await myIntercepter.get(`${conf.TR_WLMS}/api/alerts`);
  ...
  return (
    <div className=' grid h-screen grid-rows-[auto_auto_1fr]'>
      ...
      <div className='bg-black mx-4 py-2 rounded-b-md mb-4 overflow-scroll no-scrollbar  px-4'>
        ...
        <div className='grid grid-cols-9 font-semibold capitalize px-4 text-xs md:text-base items-center  text-start'>
          <p>Sr. No</p>
          <p>location</p>
          <p>KM</p>
          <p>Designation</p>
          ...
          {loading ? (
            ...
            <div className='text-white rounded-md min-w-[780px]'>
              {filteredAlerts.map((alert, index) => (
                <div key={alert.uid} className=' px-4 text-start text-xs md:text-base grid grid-cols-9 border-b border-gray-600 items-center py-1 '>
      ...
      {isModalOpen && (
        <Modal isOpen={isModalOpen}>
          <div className='w-[68vw] bg-black px-8 py-4'>
            <TRWLMSAlertForm onClose={() => setIsModalOpen(false)} />
          </div>
        </Modal>
      )}
      {isUpdateModalOpen && selectedAlert && (
        <Modal isOpen={isUpdateModalOpen}>
          <div className='w-[68vw] bg-black px-8 py-4'>
            <TRWLMSUpdateAlertForm onClose={() => setIsUpdateModalOpen(false)} alertData={selectedAlert} />
          </div>
        </Modal>
      )}
