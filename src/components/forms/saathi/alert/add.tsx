...
interface AddAlertProps {
  ...
  showRx: boolean;      // Define showRx as a boolean
}

const AddAlert:React.FC<AddAlertProps> = ({ onClose = () => { } ,showRx }) => {
  ...
  const fetchAlerts = async () => {
    try {
      if (device_id) {
        const route = showRx ? conf.SAATHI_RX : conf.SAATHI_TX;
        const response = await myIntercepter.get(`${route}/api/alerts/${device_id}`);
            ...
            <div className={`mb-4`}>
              ...
              <div className='bg-gray-800 text-white py-1 rounded-md pr-2'>
                ...
                >
                  ...
                  { devices &&  devices.filter((device)=>device.is_fixed).map(device => (
