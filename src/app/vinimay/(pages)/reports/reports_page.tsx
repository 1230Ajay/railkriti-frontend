...
const Reports: React.FC = (): JSX.Element => {
  ...
  const [logType, setLogType] = useState<'PN' | 'device status'>('PN');
    ...
    <div className='grid grid-rows-[auto_auto_1fr] h-screen'>
            ...
            <div className="mb-4 w-full">
              ...
              <div className='bg-gray-800 text-white  py-1 rounded-md pr-2'>
                <select
                  id="logType"
                  value={logType}
                  onChange={(e) => setLogType(e.target.value as 'PN' | 'device status')}
                  className="w-full border-none text-white bg-gray-800 px-2 capitalize shadow-sm"
                  required
                >
      ...
      <div className="overflow-auto no-scrollbar bg-white mt-4 mx-4 mb-4 rounded-md py-8 px-8 space-y-8">
        ...
        <div className="uppercase font-bold text-center text-3xl font-serif border-t pt-6 border-primary">
          Track PN monitoring system (TR-WLMS)
        </div>
        ...
        <div className='capitalize  font-bold'>
          ...
          <div className="font-bold uppercase text-center col-span-2 mt-8 text-2xl">{logType == "PN" ? "PN data" : 'Device Status'}</div>
          <div className='flex justify-center text-sm space-x-1'>
            <div className=''>( From {fromDate} To {toDate} )</div>
          </div>
        </div>
        {
          logType == "PN" ? <table className="border-collapse table-auto w-full text-sm text-left border text-black mt-8">
            <thead className="border-2 font-semibold  text-primary text-center bg-gray-100">
              <tr>
                ...
                <th className="border w-4/12 p-2">PN</th>
                <th className="border w-4/12 p-2">Message</th>

              </tr>
            </thead>
            <tbody className='text-center  '>
              {data.map((log:any, index) => (
                <tr key={index}>
                  ...
                  <td className="border border-primary/50 p-2">{log.pn || "--:--"}</td>
                  <td className="border border-primary/50 p-2">{log.message}</td>
