...
const AlertForm = ({ onClose = () => { } }) => {
  ...
  const fetchAlerts = async () => {
    try {
      if (device_id) {
        const response = await myIntercepter.get(`${conf.TANK_WLMS}/api/alerts/${device_id}`);
      ...
      <div className="mt-8">
        ...
        <div className="h-48 overflow-y-auto no-scrollbar">
          {alerts.map((alert,index) => (
            <div key={index} className="border-b border-gray-600 py-2 grid grid-cols-1 md:grid-cols-2 xl:grid-cols-7 text-white">
