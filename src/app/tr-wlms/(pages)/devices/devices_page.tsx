...
import { TRDeviceTableHeaderData } from '@/lib/data/tr-wlms/data.device-page-header';
...
const DevicePage: React.FC = (): JSX.Element => {
  ...
  const getDevice = async () => {
    try {
      const response = await myIntercepter.get(`${conf.TR_WLMS}/api/device`);
  ...
  const activateDeactivate = async (uid: string, status: boolean) => {
    try {
      const response = await myIntercepter.put(`${conf.TR_WLMS}/api/device/${uid}`, {
        uid,
        isActive: status
      });
  ...
  const columns = [
    ...
    { name: "Mobile", key: "installed_at", className: "text-start" },
  ...
  return (
    <div className=' h-[calc(100vh-80px)] grid  grid-rows-[auto_1fr] '>
      ...
      <HeaderTile title="TR Devices" onSearchChange={setSearchTerm} actions={[
      ...
      <div className="bg-black  mx-4 mb-4  overflow-scroll no-scrollbar px-4 rounded-b-md">
        <HeaderTable columns={TRDeviceTableHeaderData} />


        <div className='text-white rounded-md overflow-y-auto min-w-[720px] pb-4'>
          {filteredDevices.map((device, index) => {
            device.s_no = index + 1;
            device.installed_at = device.is_on_track ? "TRACK" : "LHS";
      ...
      <Modal isOpen={addDevicePopUpState}>
        <div className='w-[90vw] bg-black overflow-y-scroll no-scrollbar rounded-md px-8 pt-4 pb-8 lg:pb-0'>
          <TRWLMSDeviceReservationForm onClose={() => setAddDevicePopUpState(false)} />
        </div>
      </Modal>

      <Modal isOpen={updateDevicePopUpState}>
        <div className='w-[90vw] bg-black overflow-y-scroll no-scrollbar rounded-md px-8 pt-4 pb-8 lg:pb-0'>
          {selectedDevice && <TRWLMSDeviceUpdateForm device={selectedDevice} onClose={() => setUpdateDevicePopUpState(false)} />}
        </div>
      </Modal>
    </div>
  );
