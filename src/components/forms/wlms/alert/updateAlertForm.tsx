...
const UpdateAlertForm = ({ onClose = () => { }, alertData }: { onClose?: () => void, alertData?: any }) => {
    ...
    const handleSubmit = async () => {
        try {
            ...
            const response = await myIntercepter.put(`${conf.BR_WLMS}/api/alerts/${alertData.uid}`, alertData, {
                headers: {
    ...
    const handleDelete = async () => {
        ...
        try {
            const response = await myIntercepter.delete(`${conf.RAILTAAP}/api/alerts/${uid}`);
