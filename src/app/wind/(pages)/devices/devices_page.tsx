...
import WindDeviceAdd from '@/components/forms/wind/WindDeviceAdd';
import WindDeviceUpdate from '@/components/forms/wind/WindDeviceUpdate';
import { WindDeviceTableHeaderData } from '@/lib/data/wind/data.device-page-header';
...
const DevicePage: React.FC = (): JSX.Element => {
  ...
  const activateDeactivate = async (uid: string, status: boolean) => {
    try {
      ...
      const response = await myIntercepter.put(`${conf.WIND_URL}/device/${uid}`, {
        isActive: status,
      });
  ...
  const columns = [
    ...
    { name: "Bridge No", key: "name", className: "text-start" },
  ...
  return (
    <div className=' grid h-[calc(100vh-80px)] grid-rows-[auto_1fr] '>
      ...
      <div className="bg-black  mx-4 mb-4  overflow-scroll no-scrollbar px-4 rounded-b-md">
        <HeaderTable columns={WindDeviceTableHeaderData} />
      ...
      <Modal isOpen={addDevicePopUpState}>
        <div className='w-[90vw] bg-black overflow-y-scroll no-scrollbar rounded-md px-8 pt-4 pb-8 lg:pb-0'>
          <WindDeviceAdd onClose={() => setAddDevicePopUpState(false)}></WindDeviceAdd>
        </div>
      </Modal>

      <Modal isOpen={updateDevicePopUpState}>
        <div className='w-[90vw] bg-black overflow-y-scroll no-scrollbar rounded-md px-8 pt-4 pb-8 lg:pb-0'>
          {selectedDevice && <WindDeviceUpdate device={selectedDevice} onClose={() => setUpdateDevicePopUpState(false)} />}
        </div>
      </Modal>
    </div>
  );
