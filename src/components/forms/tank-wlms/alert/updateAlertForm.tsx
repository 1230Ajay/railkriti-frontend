...
const UpdateAlertForm = ({ onClose = () => { }, alertData }: { onClose?: () => void, alertData?: any }) => {
    ...
    const fetchAlertsForDevice = async (deviceId: string) => {
        try {
            const response = await myIntercepter.get(`${conf.TANK_WLMS}/api/alerts?device_uid=${deviceId}`);
    ...
    const handleSubmit = async () => {
        try {
            ...
            const response = await myIntercepter.put(`${conf.TANK_WLMS}/api/alerts/${alertData.uid}`, alertData, {
                headers: {
