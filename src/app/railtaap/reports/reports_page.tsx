...
const Reports: React.FC = ():JSX.Element => {
  ...
  const [logType, setLogType] = useState('Tempreture');
  const [fromDate, setFromDate] = useState<string>('');
  const [toDate, setToDate] = useState<string>('');
  const [data, setData] = useState<LogData[]>([]);

  const LogTypeOption = [
    'Tempreture' , 'device & sensor status'
  ]
  ...
  return (
    <div className='grid grid-rows-[auto_auto_1fr] h-screen'>
      <NavBar title={Titles.RailtaapTitle} />
      ...
      <div className="overflow-auto no-scrollbar bg-white mt-4 mx-4 mb-4 rounded-md py-8 px-8 space-y-8">
        ...
        <div className='capitalize  font-bold'>
          ...
          <div className="font-bold uppercase text-center col-span-2 mt-8 text-2xl">{logType == "Tempreture" ?"Tempreture data":'Device & Sensor Status' }</div>
          <div className='flex justify-center text-sm space-x-1'>
            <div className=''>( From {fromDate} To {toDate} )</div>
          </div>
        </div>
        {
          logType == "Tempreture" ? <table className="border-collapse table-auto w-full text-sm text-left border text-black mt-8">
